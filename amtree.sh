...
function login {
    ADMIN=$(curl -s -k -X POST -H "X-Requested-With:XmlHttpRequest" -H "X-OpenAM-Username:$AMADMIN" -H "X-OpenAM-Password:$AMPASSWD" $AM/json/authenticate | jq .tokenId | sed -e 's/\"//g')
    if [ -z $ADMIN ]; then
        echo "Failed to sign in to AM. Check AM URL and credentials."
        exit -1
    fi
}
...
function exportTree {
    TREE=$(curl -f -s -k -X GET -H "X-Requested-With:XmlHttpRequest" -H "iPlanetDirectoryPro:$ADMIN" $AM/json/realms/root/realm-config/authentication/authenticationtrees/trees/$1 | jq -c '. | del (._rev)')
    if [ -z "$TREE" ]; then
    ...
    EXPORTS="{ \"innernodes\":{}, \"nodes\":{} }"

    for each in $NODES
    do
        TYPE=$(echo $TREE | jq -r --arg NODE "$each" '.nodes | .[$NODE] | .nodeType')
        NODE=$(curl -s -k -X GET -H "X-Requested-With:XmlHttpRequest" -H "iPlanetDirectoryPro:$ADMIN" $AM/json/realms/root/realm-config/authentication/authenticationtrees/nodes/$TYPE/$each | jq '. | del (._rev)')
        ...
        if [ "$TYPE" == "PageNode" ]; then
            ...
            do
                ...
                PAGENODE=$(curl -s -k -X GET -H "X-Requested-With:XmlHttpRequest" -H "iPlanetDirectoryPro:$ADMIN" $AM/json/realms/root/realm-config/authentication/authenticationtrees/nodes/$PAGENODETYPE/$PAGENODEID | jq '. | del (._rev)')
...
function importTree {
    ...
    do
        ...
        RESULT=$(curl -s -k -X PUT --data "$NEWNODE" -H "Content-Type:application/json" -H "X-Requested-With:XmlHttpRequest" -H "iPlanetDirectoryPro:$ADMIN" $AM/json/realms/root/realm-config/authentication/authenticationtrees/nodes/$TYPE/$NEWUUID)
    ...
    curl -s -k -X PUT --data "$TREE" -H "Content-Type:application/json" -H "X-Requested-With:XmlHttpRequest" -H "iPlanetDirectoryPro:$ADMIN" $AM/json/realms/root/realm-config/authentication/authenticationtrees/trees/$ID > /dev/null
}


function usage {
    echo "Description: Export/import authentication tree and optionally rename the tree during import"
    echo "Usage: $0 ( -i treename | -e treename ) -h <AM host URL> -u <AM admin> -p <AM admin password>"
    exit
}


TASK=""
while getopts ":i:e:h:u:p:" arg; do
    case $arg in
